export default {
  async fetch(request, env) {
    ...
    const endpoint =
      "https://anvisninger.outseta.com/api/v1/billing/planfamilies";
    ...
    const family = (raw?.items || []).find(
      (pf) => pf?.Name === planFamilyName
    );
    ...
    const plans = (family?.Plans || [])
      ...
      .map((p) => ({
        ...
        maximumPeople:
          p?.MaximumPeople == null ? null : Number(p.MaximumPeople),
      }))
